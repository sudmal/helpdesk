...
# Статус транка к провайдеру (PHOENIX SIP)
TRUNK_LINE=$(echo "$CONTACTS" | grep "Contact:  SIP-TRUNK-")
if [ -n "$TRUNK_LINE" ]; then
  TRUNK_STATUS=$(echo "$TRUNK_LINE" | awk '{print $4}')
  TRUNK_RTT=$(echo "$TRUNK_LINE" | awk '{print $5}')
