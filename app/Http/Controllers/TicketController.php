...
class TicketController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        // Единая формула видимости по территориям (2026-08-04) — одна и та
        // же для ВСЕХ ролей, кроме admin (у него отдельный хардкод-байпас,
        // единственная реальная особая роль по прямому решению пользователя):
        // User::territoryScopeIds() = территории бригад пользователя ∪ его
        // личные территории (Настройки → Пользователи). Оператор ТП/
        // Начальник ТП/ПЭО/Логистика формально проходят через эту же
        // формулу, но у них ВСЕ территории проставлены в данных (бэкфилл +
        // автосинк новых территорий, см. TerritoryController::store()) —
        // поэтому они тоже видят всё, но через данные, а не через код.
        // Абонотдел — личные территории без бригады (скоуп сам собой уже,
        // никакого спецкейса не требуется). Пустой итоговый список территорий
        // (бригадир/монтажник без бригады и личных территорий) → 0 заявок,
        // не «видно всё» — это отдельно исправленный баг, найден 2026-08-04.
