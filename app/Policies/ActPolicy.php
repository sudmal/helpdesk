...
class ActPolicy
{
     ...
     * бригадир своей бригады или admin, пока акт ещё не утверждён.
     *
     ...
     * `ActService::flagMaterialsChanged()` уже заранее не выставляет
     * `materials_changed_at`, если редактирует сам создатель акта (нечего
     * подтверждать через `acknowledge`) — трогать не пришлось, это
     * единственная содержательная правка. Admin-обход добавлен тем же днём
     * чуть раньше по отдельной причине (нужно было чинить акт, когда
     * бригадир недоступен).
     */
