  ...
  <div class="rounded-lg overflow-hidden flex mb-2 shadow-sm bg-[#2A2A2A]" @click="$emit('open')">
    ...
    <div class="flex-1 p-3 min-w-0">
      <div class="flex items-center gap-1.5">
        ...
        <span v-if="request.status === 'pending' && !request.feasibility"
              class="shrink-0 text-black text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center"
              style="background:#FBBF24" title="Нужен ответ: возможно ли подключение">?</span>
