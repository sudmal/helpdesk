...
**Флаг `needs_callback`:** `true` — клиента нужно прозвонить. Устанавливается:
- автоматически при смене статуса на `rejected` (сообщить об отказе);
- автоматически на **любой** ответ монтажника через `/feasibility`, `possible`
  или `impossible` (см. ниже) — в обоих случаях нужен звонок клиенту: либо
  согласовать дату, либо сообщить об отказе.

**Важно, изменилось**: при смене статуса на `scheduled` (`PUT .../{id}`)
`needs_callback` теперь **НЕ** ставится — дата назначается уже ПОСЛЕ звонка
клиенту (оператор согласовал время и только затем вводит его в приложение),
повторный прозвон не нужен. Раньше (до 2026-07-24) ставился и на `scheduled`
тоже — если приложение на это полагалось, поведение изменилось.

Сбрасывается через `/mark-called` или при закрытии (`/close`).
...
> При смене статуса на `rejected` сервер автоматически ставит `needs_callback = true`.
