...
и это не акт, который бригадир создал сам. **С 2026-08-03 то же самое может и
admin** (раньше было исключение специально для этой роли — снято по просьбе
пользователя, нужно было чинить состав акта, когда бригадир недоступен).
Если в Android-приложении для роли admin есть отдельный интерфейс (или его
стоит добавить) — теперь запрос не получит 403 там, где раньше получал.
