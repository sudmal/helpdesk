...
- **Бригадир** (`foreman`) — утверждает акт (`approve`) и может, пока акт в
  ...
  количество/удалить) — **на любом акте своей бригады, включая акт, который
  создал сам** (например, когда сам закрывал заявку вместо недоступного
  монтажника; ограничение на редактирование собственного акта снято
  2026-08-03). После правки ЧУЖОГО акта выставляется `materials_changed_at`,
  и монтажнику стоит показать значок «есть изменения — подтвердите» на
  карточке заявки/акта, пока он не вызовет `acknowledge`. Если бригадир
  правит СВОЙ ЖЕ акт — `materials_changed_at` не выставляется (подтверждать
  нечего, `acknowledge` в любом случае доступен только создателю акта).
...
- `edit_materials` — можно добавлять/менять/удалять материалы (бригадир своей
  бригады или admin, статус `pending_foreman` — с 2026-08-03 не важно, кто
  создал акт, в том числе можно править свой же)
...
**С 2026-08-03** снят и запрет на акт, который бригадир создал сам себе, и (по
отдельному запросу) добавлен обход для `admin` — обе правки той же датой,
причины разные (первая — по ТЗ от Android-агента, вторая — нужно было чинить
состав акта, когда бригадир недоступен). Если в Android-приложении для роли
admin есть отдельный интерфейс (или его стоит добавить) — теперь запрос не
получит 403 там, где раньше получал; то же самое верно для бригадира на
собственном акте.
