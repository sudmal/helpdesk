...
### 🐛 Задача 2026-08-13 (2) — нет способа поменять тип/акцию акта после создания

Тот же живой кейс (`i-022108` / акт `iv-26081301`): заявку переоткрыли и
сменили тип на «Перекл. на PON» (акционный тип), но акт уже был создан
раньше как `type=repair` без акции — и добавить акцию было некуда: **ни
один эндпоинт API не позволяет менять `type`/`promotion_id` уже созданного
акта.** `POST/PUT/DELETE /acts/{id}/materials...` меняют только состав
материалов, не тип и не акцию. Эти два поля выставляются один раз, в
момент `POST /tickets/{id}/close` (или `.../connection-requests/{id}/close`),
и дальше недоступны для правки никаким запросом.

Живой кейс пришлось чинить прямой правкой в БД на сервере — это не
штатный путь. **Задача на будущее** (реализовывать сейчас не нужно, эндпоинта
пока нет): со стороны бэкенда понадобится способ поменять `type`/
`promotion_id` уже созданного акта, доступный бригадиру/admin, пока акт не
`completed` — по аналогии с окном редактирования материалов. Как только он
появится и попадёт в этот файл отдельным разделом — понадобится экран/
диалог в приложении для смены типа и акции на карточке акта (аналогично
уже существующей форме добавления материала).
