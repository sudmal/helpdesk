  ...
  <aside class="relative flex flex-col w-full h-full bg-[#141c2b] text-white shrink-0 text-[13px]">
    ...
    <nav class="flex-1 px-2.5 py-2 space-y-px overflow-y-auto overflow-x-hidden min-h-0">
      ...
      <NavItem v-if="can('acts.view') || can('surveys.conduct')" :href="route('acts.index')" icon="file-text" label="Акты" :collapsed="collapsed">
        <span v-if="actsAlerts.pending > 0 || surveyAlerts.due > 0" class="ml-auto flex items-center gap-1">
          <!-- Прыгающий телефон — есть абоненты, которым пора позвонить с опросом -->
          <span v-if="surveyAlerts.due > 0" class="animate-bounce" :title="`Опросов к обзвону: ${surveyAlerts.due}`">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5 text-amber-400" viewBox="0 0 24 24" fill="currentColor">
              <path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z"/>
            </svg>
          </span>
          <span v-if="actsAlerts.pending > 0"
                class="animate-pulse flex items-center justify-center min-w-[16px] h-4 px-1 rounded-full bg-red-500 text-white text-[9px] font-bold leading-none"
...
const surveyAlerts = computed(() =>
  page.props.surveyAlerts ?? { due: 0 }
)
