          ...
          <tbody>
            <tr v-for="member in members" :key="member.id"
              ...
              <td v-for="day in days" :key="day.date"
                  ...
                  :class="['text-center select-none transition-all w-9 h-9 sched-cell',
                           day.date === todayDate
                             ? 'border-l border-r border-b border-blue-300'
...
onMounted(() => {
  ...
  const todayTh = scrollRef.value.querySelector(`[data-date="${todayDate}"]`)
  if (todayTh) {
    todayTh.scrollIntoView({ block: 'nearest', inline: 'center' })
  }
})
</script>
...
/* Колонка с именем — закреплена при горизонтальной прокрутке */
.sched-name-col {
  position: sticky;
  left: 0;
  z-index: 10;
